...
# flask applies a namespace to all the endpoints coming from a blueprint so that
# multiple blueprints can define view functions with the same endpoint names without collisions
# .index is main.index, blueprint here use the shorter form,. redirects across blueprints must use the namespaced endpoint name
...
@login_required
def show_all():
  # cookie can only be set on a response object
  resp = make_response(redirect(url_for('.index')))
  # set_cookie take cookie name and the value as the first 2 arguments, max_age is option, until cookie expires in seconds
